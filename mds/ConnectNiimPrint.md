...
### 実機での動作確認メモ (2026-08-13)

持続接続化を実機（B1）で確認したところ、接続まわり（`ECONNRESET`/`EBUSY`）は解消し、一発で動いた。

一方で別の現象を観測: **4枚連続印刷したところ、3枚目あたりから印字位置が用紙の下方向にズレた。** 徐々にではなく、いきなりズレた（3枚目で急に発生し、そこから先はズレたまま、という挙動）。段階的な機械的ズレ（送り量の誤差の蓄積など）にしては不自然な起き方なので、原因未特定。持続接続化（ソケットを使い回すようになったこと）と関係あるかは不明 — 今回はこの現象自体を直してはいない。

現時点では対処保留。悪化する・再現性が高いことが分かったら調査する。疑うポイントの候補（未検証）:
- ラベルセンサ/紙送りキャリブレーションが、印刷ジョブをまたいでズレ続ける何か（プリンタ側の内部状態）
- `PrinterClientFixed.print_image()` の `SET_DIMENSION` 等のパラメータが、持続接続で使い回すうちに前回の状態を引きずっている
- そもそも持続接続と無関係（プリンタ本体・ラベルロール側の問題）
